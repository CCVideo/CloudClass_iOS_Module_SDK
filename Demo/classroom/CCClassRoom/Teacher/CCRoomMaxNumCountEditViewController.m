...
//    else if ([self.textFieldUserName.text integerValue] > [[CCStreamerBasic sharedStreamer] getRoomInfo].room_max_users)
//    {
//        [UIAlertView bk_showAlertViewWithTitle:@"" message:@"人数超过房间人数上限" cancelButtonTitle:@"知道了" otherButtonTitles:nil handler:nil];
//    }
//    else if ([[CCStreamerBasic sharedStreamer] getRoomInfo].live_status == CCLiveStatus_Start)
//    {
...
//       BOOL result = [[CCStreamerBasic sharedStreamer] changeRoomRotate:CCRotateType_Update time:count completion:^(BOOL result, NSError *error, id info) {
