...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            _timer = [NSTimer scheduledTimerWithTimeInterval:0.2 target:weakProxy selector:@selector(observeAuthrizationStatusChange) userInfo:nil repeats:YES];
        } else {
...
- (void)setAllowEdited:(BOOL)allowEdited
{
    _allowEdited = allowEdited;
    NSString *allowPickingVideoStr = _allowEdited ? @"1" : @"0";
    [[NSUserDefaults standardUserDefaults] setObject:allowPickingVideoStr forKey:@"tz_allowEdited"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

- (void)settingBtnClick {
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString]];
    /* 2019年09月24日17:45:02注释
    if (iOS8Later) {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString]];
    } else {
...
- (void)configTableView {
    ...
    [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<TZAlbumModel *> *models) {
        ...
        if (!_tableView) {
            CGFloat top = 44;
            if (iOS7Later) top += 20;
            _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, top, weakSelf.view.tz_width, weakSelf.view.tz_height - top) style:UITableViewStylePlain];
