...
- (void)viewDidLoad {
    ...
    BOOL allowEdited = [[[NSUserDefaults standardUserDefaults] objectForKey:@"tz_allowEdited"] isEqualToString:@"1"];
    if (allowEdited)
    {
        [self addClipView];
    }
...
- (void)addClipView
{
    _clipLineView = [[UIView alloc] init];
    _clipLineView.frame = CGRectMake(0, (self.view.frame.size.height - self.view.frame.size.width)/2.f, self.view.frame.size.width, self.view.frame.size.width);
    _clipLineView.backgroundColor = [UIColor clearColor];
    _clipLineView.layer.borderColor = [UIColor whiteColor].CGColor;
    _clipLineView.layer.borderWidth = 2.f;
    _clipLineView.userInteractionEnabled = NO;
    [self.view addSubview:_clipLineView];
}
...
- (void)okButtonClick {
    ...
    if (_tzImagePickerVc.selectedModels.count == 0 && _tzImagePickerVc.minImagesCount <= 0) {
        ...
        if (allowEdited)
        {
            ...
            CGRect clipRect = _clipLineView.frame;
            clipRect = [self.view convertRect:clipRect toView:cell.imageContainerView];
            UIImage *image = [TZPhotoPreviewController shotWithView:cell.imageContainerView scope:clipRect];
